...
def conn():
    ...
    try:
        ...
        eel.showText("Connection Successfull !!")
    except:
...
def upld(file_path):
    ...
    try:
        ...
        eel.file_not_uploaded()
        return
    temp = file_path.split('filepath\\')
    filename = temp[1]
    try:
        # Make upload request
    ...
    try:
        ...
        fnsize=len(filename)
        s.send(str(fnsize).encode(FORMAT))
        res=bytes(filename,FORMAT)
        s.send(res)
        # Wait for server ok then send file size
        s.recv(BUFFER_SIZE).decode(FORMAT)
        fsize=os.stat(file_path)
        s.send(str(fsize.st_size).encode(FORMAT))
    except:
    ...
    try:
        ...
        eel.file_successfully_uploaded(filename, upload_time, upload_size)
    except:
...
def list_files(purpose):
    ...
    try:
        ...
        for i in range(int(number_of_files)):
            ...
            fsize.append(file_size)
        # Get total size of directory
        total_directory_size = struct.unpack("i", s.recv(4))[0]
        print("Total directory size: {}b".format(total_directory_size))
        if (purpose == "list"): 
            eel.showlist(fname, fsize, total_directory_size)
        elif (purpose == "delete"): 
            eel.delete_file(fname)
        elif (purpose == "download"):
            eel.download_file(fname)
...
def dwld(file_name):
    ...
    try:
        ...
        s.send(str(sys.getsizeof(file_name)).encode(FORMAT))
        res=bytes(file_name,FORMAT)
        s.send(res)
        # Get file size (if exists)
        file_size = struct.unpack("i", s.recv(4))[0]
        print (file_size)
        if file_size == -1:
     # If file size -1, the file does not exist
    ...
    try:
       ...
       # Enter loop to recieve file
        output_file = open("D:\\filepath\\"+file_name, "wb")
        bytes_recieved = 0
        print ("\nDownloading...")
        ...
        # tempText += "Successfully Downloaded "+file_name
        ...
        # tempText += "\nTime elapsed: "+time_elapsed+"s\nFilesize: "+file_size+"b"
        eel.showText("Successfully Downloaded "+file_name+" !!!")
    except:
        print("Error downloading file")
        eel.showText("Error downloading file")
        return
    return
...
def delf(file_name):
    ...
    try:
        ...
        if file_exists == -1:
            ...
            eel.showText("The file does not exist on server")
            return
    except:
    ...
    try:
        ...
        if delete_status == 1:
            ...
            eel.showText("File successfully deleted")
            return
        else:
